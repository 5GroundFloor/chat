...
let wheelOptions = ["选项一", "选项二", "选项三", "选项四"];
let wheelResultText = "";
...
function initDecisionModule() {
    ...
    const sendChoiceBtn = document.getElementById('send-choice-btn');
    ...
    if (sendChoiceBtn && !sendChoiceBtn.dataset.initialized) {
        sendChoiceBtn.addEventListener('click', () => {
            const questionInput = document.getElementById('wheel-question');
            ...
            const validOptions = wheelOptions.filter(o => o.trim());
            if (validOptions.length < 2) {
                showNotification('选项不能为空', 'warning');
                return;
            }
            // 发送抉择消息
            sendMessage('', 'choice', { choiceQuestion: question, choiceOptions: validOptions });
            hideModal(document.getElementById('wheel-modal'));
            // 重置
            if (questionInput) questionInput.value = '';
            wheelResultText = "";
        });
        sendChoiceBtn.dataset.initialized = 'true';
    }
}
