...
function manageAutoSendTimer() {
    ...
    if (settings.autoSendEnabled) {
        ...
        window.handleChoiceSelect = function(btn, msgId, option) {
            const msgIndex = messages.findIndex(m => String(m.id) === String(msgId));
            if (msgIndex === -1) return;
            const msg = messages[msgIndex];
            if (msg.choiceLocked) return;
            
            // 标记为已锁定
            msg.choiceLocked = true;
            msg.choiceSelected = option;
            
            // 禁用所有按钮
            const allBtns = document.querySelectorAll(`[data-msg-id="${msgId}"] .choice-option-btn`);
            ...
            // 对方发起的抉择 -> 用户自己选择
            if (msg.sender === 'user') {
                sendMessage(`${settings.partnerName || '对方'} 选择了：${option}`, 'normal');
            } else {
                sendMessage(`${settings.myName || '我'} 选择了：${option}`, 'normal');
            }
...
function createMessageFragment(msg, prevMsg, nextMsg, lastSenderRef) {
    ...
    if (msg.type === 'choice') {
        ...
        choiceDiv.className = 'choice-message';
        const question = msg.choiceQuestion || '请选择：';
        const options = msg.choiceOptions || [];
        const colors = ['#FFD93D','#FF6B6B','#6BCB77','#4D96FF','#E0C3FC','#FF9A8B','#A8D8EA','#C44569'];
        const optionsHTML = options.map((opt, i) => {
            const color = colors[i % colors.length];
            const isSelected = msg.choiceSelected === opt;
            return `<button class="choice-option-btn ${isSelected ? 'selected' : ''}" 
                style="${isSelected ? `background:${color};color:#fff;border-color:${color};` : `border-color:${color};color:${color};`}"
                data-option="${opt.replace(/"/g, '&quot;')}" 
                ${isSelected || msg.choiceLocked ? 'disabled' : ''}
                onclick="handleChoiceSelect(this, '${msg.id}', '${opt.replace(/'/g, "\\'")}')">${opt}</button>`;
        }).join('');
        choiceDiv.innerHTML = `
...
const addMessage = (message) => {
    ...
    if (message.sender === 'user' && message.type === 'normal' && typeof window._onUserMessage === 'function') {
        ...
        function sendMessage(textOverride = null, type = 'normal', extraData = {}) {
            const text = textOverride || DOMElements.messageInput.value.trim();
            const imageFile = DOMElements.imageInput.files[0];
            if (!text && !imageFile && type === 'normal' && !extraData.choiceOptions) return;
